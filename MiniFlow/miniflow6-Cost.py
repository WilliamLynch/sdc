...
class MSE(Layer):
    ...
    def forward(self):
        ...
        y is target output and a is output computed by the neural network. 
        We then square the difference. 
        Lastly we need to sum the squared differences 
        and divide by the total number of examples m. 
        ...
        diff = y - a
        self.value = np.sum(np.square(diff))/m
